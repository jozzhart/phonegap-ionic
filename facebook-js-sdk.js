...
FB.provide('', {
    ...
    ui: function(params, cb) {
        ...
        if (displayName === 'dialog') {
            // TODO remove once all dialogs are on
            // uiserver
            displayName = 'iframe';
    ...
    staticAuthCheck: function(lsCb) {
        ...
        FB.Content.insertIframe({
            ...
            onload: function(iframe) {
                ...
                FB.JSON.stringify({
                    method: 'getItem',
                    params: ['LoginInfo_' + FB._apiKey, /* do_log */
                    true],
                    returnCb: guid
                }),
                domain);
...
FB.provide('URI', {
    ...
    resolve: function(uri) {
        if (!uri) {
            // IE handles this case poorly, so we will be explicit about it
...
FB.provide('Anim', {
    ...
    ate: function(dom, props, duration, callback) {
        ...
        timer = setInterval(FB.bind(function() {
            ...
            if (pd == 1) {
                // are we done? clear the timer, call the callback
                clearInterval(timer);
                if (callback) {
                    callback(dom);
                }
...
FB.subclass('XFBML.ConnectBar', 'XFBML.Element', null, {
    ...
    _page: null,
    // the external site's content parent node
    _displayed: false,
    // is the bar currently displayed
    _notDisplayed: false,
    // is the bar currently not displayed
    _container: null,
    _animationSpeed: 0,
    // default to no (zero time, instant) animation
        ...
        FB.getLoginStatus(this.bind(function(resp) {
            FB.Event.monitor('auth.statusChange', this.bind(function() {
                ...
                if (this.isValid() && FB._userStatus == 'connected') {
                    ...
                    FB.api({
                        // check if marked seen / current seen count
...
FB.subclass('XFBML.LoginButton', 'XFBML.ButtonElement', null, {
    ...
    setupAndValidate: function() {
        ...
        if (this._attr.registration_url) {
            ...
            this._saveStatus(this.process, /* on_click */
            false));
            // Change the displayed HTML immediately
            FB.getLoginStatus(this._saveStatus(this.process, /* on_click */
            false));
        }
        ...
        if (!this._attr.registration_url) {
            ...
            case 'unknown':
                ...
                FB.bind(function(response) {
                    ...
                    this._saveStatus(this._authCallback, /* on_click */
                    true),
                    /* force */
                    true), this);
                }, this)
